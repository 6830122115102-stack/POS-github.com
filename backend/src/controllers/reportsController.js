...
class ReportController {
  ...
  async getSalesSummary(req, res) {
    try {
      ...
      const summary = await this.saleRepository.queryOne(sql, params);
  ...
  async getTopProducts(req, res) {
    try {
      ...
      const products = await this.saleRepository.query(sql, params);
  ...
  async getSalesByPeriod(req, res) {
    try {
      ...
      const data = await this.saleRepository.query(sql, params);
  ...
  async getDashboardStats(req, res) {
    try {
      // Today's sales
      const todaySales = await this.saleRepository.queryOne(`
      ...
      // This month's sales
      const monthSales = await this.saleRepository.queryOne(`
        SELECT COUNT(*) as count, COALESCE(SUM(total_amount), 0) as total
        FROM sales
      ...
      const lowStock = await this.productRepository.queryOne(`
      ...
      // Total customers
      const totalCustomers = await this.customerRepository.queryOne(`
        SELECT COUNT(*) as count FROM customers
      `);

      // Total products (only valid products: active, price > 0, stock > 0)
      const totalProducts = await this.productRepository.queryOne(`
        SELECT COUNT(*) as count FROM products WHERE is_active = 1 AND price > 0 AND stock_quantity > 0
      `);

      // Recent sales
      const recentSales = await this.saleRepository.query(`
  ...
  async exportSalesCSV(req, res) {
    try {
      ...
      const sales = await this.saleRepository.query(sql, params);
  ...
  async exportSalesPDF(req, res) {
    try {
      ...
      const summary = await this.saleRepository.queryOne(summarySQL, summaryParams);
